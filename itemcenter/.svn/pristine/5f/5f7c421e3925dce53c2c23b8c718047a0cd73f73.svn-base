...
public class GlobalPropertyManagerImpl implements GlobalPropertyManager {
	...
	@Resource
	private GlobalPropertyDAO globalPropertyValueDAO;
	...
	public boolean updateGlobalProperty(GlobalPropertyDTO globalPropertyDTO) throws ItemException {
		...
		if (num > 0) {
			...
			throw ExceptionUtil
					.getException(ResCodeNum.SYS_E_DB_UPDATE, "update global_property error-->primary id:"
	...
	public GlobalPropertyDTO getGlobalProperty(Long id) throws ItemException {
		...
		if (globalPropertyDO == null) {
			throw ExceptionUtil
					.getException(ResCodeNum.BASE_PARAM_E_RECORD_NOT_EXIST, "requested record doesn't exist from table global_property-->id:"
							+ id);
		}
	...
	public boolean deleteGlobalProperty(Long id) throws ItemException {
		int num = globalPropertyValueDAO.deleteGlobalProperty(id);
		if (num > 0) {
			return true;
		} else {
			throw ExceptionUtil
					.getException(ResCodeNum.SYS_E_DB_DELETE, "delete global_property error-->primary id:" + id);
		}
	}
	...
	private void verifyNewAddedGlobalPropertyDTOProperty(GlobalPropertyDTO globalPropertyDTO) throws ItemException {
		...
		if (globalPropertyDTO.getPropertyName() == null) {
			throw ExceptionUtil.getException(ResCodeNum.PARAM_E_MISSING, "property_name is null");
		}
		if (globalPropertyDTO.getPropertyType() == null) {
			throw ExceptionUtil.getException(ResCodeNum.PARAM_E_MISSING, "property_type is null");
		}
	...
	private void verifyUpdatedGlobalPropertyDTOProperty(GlobalPropertyDTO globalPropertyDTO) throws ItemException {
		...
		if (globalPropertyDTO.getId() == null) {
			throw ExceptionUtil.getException(ResCodeNum.PARAM_E_MISSING, "globalProperty id is null");
		}
	}
