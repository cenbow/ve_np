...
@Service
public class AuditActivityAction implements Action<MarketingActivityDTO> {
	private static final Logger log = LoggerFactory.getLogger(AuditActivityAction.class);
	...
	public MarketingResponse<MarketingActivityDTO> execute(
		...
		return transactionTemplate.execute(new TransactionCallback<MarketingResponse<MarketingActivityDTO>>(){
			...
			public MarketingResponse<MarketingActivityDTO> doInTransaction(TransactionStatus arg0) {
				...
				try {
					activityManager.updateActivityStatus(id,MarketingConstant.ACT_STATUS_AUDITING);
	...
	private MarketingTrackDTO getTrackDto(Long id,Long userId){
		...
		track.setAuditType(MarketingConstant.AUDIT_TYPE_APPLY);
	...
	public String getName() {
		return ActionEnum.ACTIVITY_SUBMIT_AUDTING.getActionName();
	}
}
