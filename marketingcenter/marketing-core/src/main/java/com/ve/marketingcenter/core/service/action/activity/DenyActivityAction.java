...
@Service
public class DenyActivityAction implements Action<MarketingActivityDTO> {
	private static final Logger log = LoggerFactory.getLogger(DenyActivityAction.class);
	...
	public MarketingResponse<MarketingActivityDTO> execute(
		...
		return transactionTemplate.execute(new TransactionCallback<MarketingResponse<MarketingActivityDTO>>(){
			...
			public MarketingResponse<MarketingActivityDTO> doInTransaction(TransactionStatus arg0) {
				...
				try {
					activityManager.updateActivityStatus(id,MarketingConstant.ACT_STATUS_DENIED);
	...
	public String getName() {
		return ActionEnum.ACTIVITY_AUDIT_DENIED.getActionName();
	}
}
