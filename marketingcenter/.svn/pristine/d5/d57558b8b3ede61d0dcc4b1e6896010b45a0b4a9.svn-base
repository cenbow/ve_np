...
@Service
public class CancelActivityAction implements Action<MarketingActivityDTO> {
	private static final Logger log = LoggerFactory.getLogger(CancelActivityAction.class);
	...
	public MarketingResponse<MarketingActivityDTO> execute(
		...
		return transactionTemplate.execute(new TransactionCallback<MarketingResponse<MarketingActivityDTO>>(){
			...
			public MarketingResponse<MarketingActivityDTO> doInTransaction(TransactionStatus arg0) {
				...
				try {
					activityManager.updateActivityStatus(id,MarketingConstant.ACT_STATUS_INVALID);
	...
	public String getName() {
		return ActionEnum.ACTIVITY_CANCEL.getActionName();
	}
}
