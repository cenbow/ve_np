package com.ve.deliverycenter.core.filter.conf;
...
public class FilterConfParser {
	...
	public FilterConfBean parseConfig(String filterConfFileName)
			throws Exception {
		// 载入filter配置文件
		...
		NodeList nodeList = doc
				.getElementsByTagName(FilterConfElementEnum.ELEMENT_CHAIN
						.getValue());
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			chainMap.put(chainElement
					.getAttribute(FilterConfElementEnum.ELEMENT_ATTRBUTE_NAME
							.getValue()), chain);
		}

		nodeList = doc
				.getElementsByTagName(FilterConfElementEnum.ELEMENT_EXTENTION
						.getValue());
		for (int i = 0; i < nodeList.getLength(); i++) {
	...
	private FilterDescBean parseFilter(Element filterElement) {
		...
		String v = filterElement
				.getAttribute(FilterConfElementEnum.ELEMENT_ATTR_FACTORY
						.getValue());
		if (v != null && v.trim().length() > 0)
			filterDesc.setFactoryClass(v.trim());
		v = filterElement
				.getAttribute(FilterConfElementEnum.ELEMENT_ATTR_IMPL_CLASS
						.getValue());
		if (v != null && v.trim().length() > 0)
			filterDesc.setImplClass(v.trim());
		v = filterElement
				.getAttribute(FilterConfElementEnum.ELEMENT_ATTR_INTERFACE
						.getValue());
		if (v != null && v.trim().length() > 0)
			filterDesc.setIntfClass(v);
		v = filterElement
				.getAttribute(FilterConfElementEnum.ELEMENT_ATTR_METHOD
						.getValue());
		if (v != null && v.trim().length() > 0)
			filterDesc.setMethodName(v);
		v = filterElement
				.getAttribute(FilterConfElementEnum.ELEMENT_ATTR_EXCLUSION
						.getValue());
		if (v != null && v.trim().length() > 0)
