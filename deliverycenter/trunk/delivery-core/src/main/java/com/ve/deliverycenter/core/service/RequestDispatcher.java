...
import com.ve.deliverycenter.common.constant.ParamEnum;
import com.ve.deliverycenter.common.constant.RetCodeEnum;
import com.ve.deliverycenter.core.exception.DeliveryException;
import com.ve.deliverycenter.core.filter.FilterChain;
import com.ve.deliverycenter.core.filter.FilterManager;
import com.ve.deliverycenter.core.service.action.Action;
import com.ve.deliverycenter.core.service.action.ActionHolder;
...
public class RequestDispatcher {
	...
	public DeliveryResponse dispatch(DeliveryRequest req) {
		if (req == null) {
	...
	private String getAppCode(RequestContext context, DeliveryRequest req) {
		// TODO impl
		String appCode = null;
	 ...
	 * </pre>
	 */
	class ActionCall implements Callable<DeliveryResponse> {

		private DeliveryRequest req;

		public ActionCall(DeliveryRequest req) {
			super();
			this.req = req;
		}

		@Override
		public DeliveryResponse call() {
			...
			if (null != action) {
				...
				DeliveryResponse re = new DeliveryResponse(RetCodeEnum.SUCCESS);
				// 获取appCode
				String appCode = getAppCode(context, req);
				if (!allowAccess(appCode, action)) {
					re = new DeliveryResponse(RetCodeEnum.REQUEST_FORBBIDEN);
					return re;
				}
				...
				try {
					...
					res = new DeliveryResponse(e.getCode(), e.getMessage());
					log.error("do action:" + req.getCommand()
							+ " occur Exception:", e);
					return res;
				} catch (Exception e) {
					res = new DeliveryResponse(RetCodeEnum.SERVICE_EXCEPTION);
					log.error("do action:" + req.getCommand()
							+ " occur Exception:", e);
					return res;
				} finally {
					if (System.currentTimeMillis() % 128 == 1) {
						log.info("result:" + res.isSuccess()
								+ ",filterBeforeCost:"
